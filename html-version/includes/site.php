...
$clients = [
    ...
    ["name" => "BRASOL", "url" => "https://www.brasol.co/", "logo" => "assets/logo-novo-cliente.svg"],
];
...
$gallery_images = [
    ...
    ["src" => "assets/projeto-antigo-4.jpg", "alt" => "Projeto fotovoltaico em cobertura comercial"],
    ["src" => "assets/projeto-antigo-5.jpg", "alt" => "Instala\u{00E7}\u{00E3}o solar em estrutura industrial vista a\u{00E9}rea"],
    ["src" => "assets/projeto-antigo-6.jpg", "alt" => "Estacionamento com cobertura de energia solar"],
...
<header class="site-header<?= $is_home ? ' site-header-home' : '' ?>" data-home="<?= $is_home ? 'true' : 'false' ?>">
  <div class="container">
    <nav class="navbar<?= $is_home ? ' navbar-home' : '' ?>">
      ...
      <button class="menu-toggle menu-toggle-home" type="button" aria-expanded="false" aria-controls="mobile-menu"><span></span><span></span><span></span></button>
      <ul class="nav-links nav-desktop">
        <?php foreach ($nav_items as $item): ?>
          <li><a class="<?= $current_page === $item["href"] ? "active " : "" ?>nav-link-home" href="<?= htmlspecialchars($item["href"]) ?>"><?= htmlspecialchars($item["label"]) ?></a></li>
        <?php endforeach; ?>
      </ul>
      <a class="nav-cta nav-cta-home desktop-only" href="https://wa.me/<?= htmlspecialchars($site_phone_link) ?>" target="_blank" rel="noopener noreferrer">Entre em contato</a>
    </nav>
